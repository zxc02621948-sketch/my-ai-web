...
export async function DELETE(req) {
  ...
  try {
    ...
    const directKey = String(body?.originalImageKey || "").trim();
    const fromUrl = extractR2KeyFromUrl(body?.originalImageUrl);
    const key = directKey || fromUrl;

    if (!key) {
      return NextResponse.json({ success: false, message: "缺少 originalImageKey 或 originalImageUrl" }, { status: 400 });
    }

    // 僅允許刪除自己上傳路徑，避免濫刪他人檔案
    const expectedPrefix = `images/original/${String(currentUser._id)}/`;
    ...
    console.error("❌ 刪除 R2 原圖失敗:", error);
