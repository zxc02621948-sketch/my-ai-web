...
export function PlayerProvider({
  ...
  // ✅ forceRefresh: 是否強制刷新 artwork（添加時間戳），用於從暫停恢復播放時
  const updateMediaSessionMetadata = useCallback((trackOverride = null, forceRefresh = false) => {
    ...
    // 1. 優先使用 trackOverride
    if (trackOverride) {
      coverCandidate = trackOverride.coverImageUrl || trackOverride.cover || trackOverride.imageUrl || trackOverride.thumbnailUrl || "";
    }
    
    // 2. 如果沒有，使用 currentTrack
    if (!coverCandidate && currentTrack) {
      coverCandidate = currentTrack.coverImageUrl || currentTrack.cover || currentTrack.imageUrl || currentTrack.thumbnailUrl || "";
    }
    ...
    if (!coverCandidate) {
      ...
      if (Array.isArray(list) && list.length > 0 && idx >= 0 && idx < list.length) {
        const trackFromPlaylist = list[idx];
        if (trackFromPlaylist) {
          coverCandidate = trackFromPlaylist.coverImageUrl || trackFromPlaylist.cover || trackFromPlaylist.imageUrl || trackFromPlaylist.thumbnailUrl || "";
        }
      }
    }
    ...
    if (coverCandidate && forceRefresh) {
      try {
  ...
  const onPlay = useCallback(() => {
    ...
    // ✅ 更新 Media Session metadata（確保封面立即顯示）
      ...
      try {
        ...
        if (wasPaused) {
          ...
          if (currentTrackForResume) {
            // ✅ 關鍵修復：在重置前先保存現有的 artwork（如果有的話）
            let savedArtwork = null;
            try {
              const existingMetadata = navigator.mediaSession.metadata;
              if (existingMetadata && existingMetadata.artwork && existingMetadata.artwork.length > 0) {
                savedArtwork = existingMetadata.artwork;
                console.log("[MediaSession] 🔍 保存現有的 artwork:", savedArtwork.map(a => a.src));
              }
            } catch (e) {
              // 忽略錯誤
            }
            ...
            try {
              ...
              console.log("[MediaSession] 🔍 已重置 Media Session 狀態");
            } catch (e) {
            ...
            setTimeout(() => {
              if (audioRef.current && !audioRef.current.paused && navigator.mediaSession) {
                ...
                if (savedArtwork && savedArtwork.length > 0) {
                  try {
                    // ✅ 從保存的 artwork 中提取 URL，並設置到 track 對象中（如果 track 中沒有）
                    const artworkUrl = savedArtwork[0]?.src;
                    if (artworkUrl && (!currentTrackForResume.coverImageUrl || currentTrackForResume.coverImageUrl === "")) {
                      currentTrackForResume.coverImageUrl = artworkUrl;
                      console.log("[MediaSession] 🔍 從保存的 artwork 中提取 URL 並設置到 track:", artworkUrl);
                    }
    ...
    // ✅ Android 背景播放：等待音頻準備好後設置 position state
    const setupMediaSessionForAndroid = (attempts = 0) => {
      ...
      if (!audio.paused && audio.duration > 0 && isFinite(audio.duration)) {
        try {
          // ✅ 設置 position state（Android 需要，確保進度條正確顯示）
  ...
  const onPause = useCallback(() => {
    ...
    if (typeof navigator !== "undefined" && navigator.mediaSession && audioRef.current) {
      try {
        ...
        // ✅ 更新 metadata 確保封面不會丟失
        updateMediaSessionMetadata();
        
        // ✅ 設置 playbackState 和 position state
        setTimeout(() => {
          try {
  ...
  useEffect(() => {
    ...
    if (hasMusic) {
      ...
      // ✅ 總是先設置基本的 metadata（確保立即顯示，即使信息不完整）
      try {
        // ✅ 確保 metadata 有有效的 title 和 artist（Android 和 iOS 都需要）
      ...
      // ✅ 如果需要獲取完整信息（播放列表中缺少封面或作者），且 URL 是音樂流媒體 URL，異步獲取並更新
      ...
      if (needsMusicInfo) {
        ...
        if (musicId) {
          // ✅ 使用 sessionStorage 緩存音樂信息，避免重複請求
            ...
            if (cachedMusic) {
              try {
                ...
                if (music.coverImageUrl) {
                  cachedArtwork.push({ src: music.coverImageUrl, sizes: "96x96", type: "image/png" });
                  cachedArtwork.push({ src: music.coverImageUrl, sizes: "128x128", type: "image/png" });
                  cachedArtwork.push({ src: music.coverImageUrl, sizes: "192x192", type: "image/png" });
                  cachedArtwork.push({ src: music.coverImageUrl, sizes: "256x256", type: "image/png" });
                  cachedArtwork.push({ src: music.coverImageUrl, sizes: "384x384", type: "image/png" });
                  cachedArtwork.push({ src: music.coverImageUrl, sizes: "512x512", type: "image/png" });
                }
    ...
    // ✅ 更新播放狀態（metadata 已在上面設置）
    // ✅ 注意：playbackState 應該由 onPlay/onPause 控制，這裡只作為備份同步
    if (typeof navigator !== "undefined" && navigator.mediaSession) {
      try {
        // ✅ 只在狀態不一致時更新（避免與 onPlay/onPause 衝突）
