...
export async function createAllIndexes() {
  ...
  // 圖片分析集合索引（優化查詢性能）
  await ImageAnalytics.collection.createIndex({ imageId: 1, createdAt: -1 });
  await ImageAnalytics.collection.createIndex({ sessionId: 1, createdAt: -1 });
  await ImageAnalytics.collection.createIndex({ eventType: 1, createdAt: -1 });
  await ImageAnalytics.collection.createIndex({ userId: 1, createdAt: -1 });
  await ImageAnalytics.collection.createIndex({ createdAt: -1 }); // TTL 索引（自動刪除）
